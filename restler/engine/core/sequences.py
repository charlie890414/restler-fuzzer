...
class Sequence(object):
    ...
    def replay_sequence(self, candidate_values_pool=None):
        ...
        """
        def send_and_parse(request_data, request_in_collection):
        ...
        SequenceTracker.initialize_sequence_trace(self.combination_id, tags={'origin': 'replay'})
        ...
        for request_data in self._sent_request_data_list[:-1]:
            ...
            request_in_collection = request_data.find_matching_request(self.requests)
            if request_in_collection is None:
                print("WARNING: replay request not found in sequence request collection")

            _ = send_and_parse(request_data, request_in_collection)

        final_request_data = self._sent_request_data_list[-1]
        request_in_collection = final_request_data.find_matching_request(self.requests)
        if request_in_collection is None:
            print("WARNING: replay request not found in sequence request collection")

        # Send final request and return its status code
        status_code = send_and_parse(final_request_data, request_in_collection)
        SequenceTracker.clear_sequence_trace()
        return status_code
